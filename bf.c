            ...
            const Blowfish_Key key)
{
    register const __u32 *p=(const __u32 *)key;
    register unsigned long
        l = __be32_to_cpup((__u32 *)dataIn)^(*p++),
        r = (unsigned long)__be32_to_cpup((__u32 *)(dataIn+4));
    RoundsP;
            ...
            const Blowfish_Key key)
{
    ...
    register unsigned long
    l=__be32_to_cpup((__u32 *)dataIn)^(*p--),
    r=__be32_to_cpup((__u32 *)(dataIn+4));
    RoundsN;
    ((__u32 *)dataOut)[0]=__cpu_to_be32(r^(*p));
    ((__u32 *)dataOut)[1]=__cpu_to_be32(l);
}

void Blowfish_ExpandUserKey(const unsigned char *userKey, int userKeyLen,
                Blowfish_Key key)
{
