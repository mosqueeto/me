...
/* "codelist" is a separate list of all the machine code objects */
VAR *codelist = (VAR *)NULL;

/* THREADs are executable arrays of objects */
...
OBJECT *ob;
{
    ...
    if( is_string(ob) ){
        ...
        sprintf(&buf[i],"%s:",ob->u.v->name);
        mlwrite(buf);
        if( ob == ob->u.v->ob ) printf("Circular ob!\n");
        else pr_ob(indent+3,ob->u.v->ob);
    } else {
...
{
    VAR *v;
    if( ( v = _lookup( &codelist,name ) ) ) return v;
    else return _lookup( &namelist,name );
}

VAR *insert_ob(list,name,ob)
VAR **list;
char *name;
OBJECT *ob;
{
...
int flags;
{
    ...
    if( insert_ob(&codelist,name,ob) ) return ob;
    else {
...
void divide() { arithmetic_op(OP_DIV); }
...
// get a blank delimited word from the input stream
char *get_word()
{
    char c,nc;
    ...
    wx++;
    if( word[0] == START_STRING ){ //### what is this? allows <a>? 
        word[1] = '\0';
        return word;
    }
...
OBJECT *ob;
{
    ...
    if( is_code(ob) ) {
        ...
        if( execing_thread ) push( ob );
        else exec_thread(ob->u.t);
    }
    else if( is_name(ob) ) {
        exec( ob->u.v->ob );
    }
...
void interp()
{
    ...
    while( TRUE ){
        ...
        if( w[0] == TIC ) {
            if( (v = lookup(&w[1])) == NULL ) {
                v = insert_ob(&namelist,&w[1],&undefined_ob);
            }
            ob = new_ob("interp:var");
            ob->flags = OB_NAME;
            ob->u.v = v;
            push(ob);
            continue;
...
void set()
{
    ...
    if( (v = lookup(name)) == (VAR *)NULL ){
        (void)insert_ob(&namelist,name,ob);
    } 
    else {
        if( is_code(v->ob) ){
...
void init()
{
    ...
    install_code(".x",x_exec,0);
    install_code("rpt",rpt,0);
    install_code("def",def,0);
    install_code("set",set,0);
    ...
    install_code("codelist",dump_codelist,0);
...
// instantiate the "undefined object"
