...
extern void Blowfish_Encrypt(void *dataIn, void *dataOut,
			       const Blowfish_Key key);
extern void Blowfish_Decrypt(void *dataIn, void *dataOut,
			       const Blowfish_Key key);
...
void Blowfish_Encrypt(void *dataIn, void *dataOut,
			const Blowfish_Key key)
{
	register const __u32 *p=(const __u32 *)key;
	register unsigned long
...
void Blowfish_Decrypt(void *dataIn, void *dataOut,
			const Blowfish_Key key)
{
	register const __u32 *p=((const __u32 *)key)+17;
	register unsigned long
...
//	close(r);
	...
	o = 0;
	while( ! done ) {
		n = read(in,&buf[8],32768);
		// null pad to an 8 byte boundary
		m = n + 8;
		if( n < 32768 ) {
			pad_size = (8 - (n%8))%8;
		...
		if( n != m - o ) die("write error\n");

		// copy last enc block to be IV for next buffer
		for(j=0,k=i;j<8;j++,k++ ) buf[j] = buf[k];
...
decrypt(char *pw,int in,int out)
{
	...
	unsigned char t[8];
	int i,j,k,l,m,n,o,pad_size,done;
	...
	while( ! done ) {
		...
		if( done ) {
			i -= (8 + buf[i]); // last byte contains pad_size
			m = write(out,buf,i);
			if(m != i ) die("write error\n");
			break;
		}

		m = write(out,buf,i-24); // funky stuff to determin eof
		if(m != i-24 ) die("write error\n");

		//save IV and potential pad blocks
		for( k=0,j=i-24;j<i;j++,k++ ) buf[k] = buf[j];
		o = 24;
	}	
}
...
char *argv[];
{
	...
	if( argc-optind < 2 ) usage();
	if( strlen(argv[optind]) > 250 ) die("input file name too long");
	if( strlen(argv[optind+1]) > 250 ) die("output file name too long");
	...
	out = open(argv[optind+1],O_WRONLY|O_CREAT,0600);
	if( out<0 ) {
